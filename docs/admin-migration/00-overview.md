...
| D16 | `DocumentCategory`'s **value is the Hungarian slug** (`tajekoztato`, `probaverzio`, `programfrissites`, `dokumentacio`, `egyeb`), not `integra-*`. One vocabulary covers the stored column, the API path, the SPA route and the storage directory. | Task 16 shipped `integra/{category}` with the enum's `integra-*` values, which created a second set of folders next to the seeded Hungarian ones (`integra/integra-other` beside `integra/egyeb`) and would have split further once Task 27 imports legacy data. Making the slug the value fixes the directory for free, drops the slug↔category map from `types/integra.ts` (`IntegraCategory` and `IntegraCategorySlug` collapse into one type), and lets `BreadcrumbService` share `INTEGRA_CATEGORY_LABELS`. No migration: the app is not live and the only data is seeded locally, so re-seeding covers the rename. |
...
| 05 | FE | [News admin grid](05-fe-news-list.md) | 03, 04 | [x] |
| 06 | FE | [News create/edit form](06-fe-news-form.md) | 05 | [x] |
| 07 | BE | [Offers admin API](07-be-offers-crud.md) | 01 | [x] |
...
| 19 | FE | [Users admin grid](19-fe-users-list.md) | 03, 18 | [x] |
