...
describe('Get Admin Knowledgebase Item', function () {
    beforeEach(function () {
        Carbon::setTestNowAndTimezone('2026-02-28 21:59:40', 'Europe/Budapest');
        resetGetAdminKnowledgebaseItemTestData();
    });

    test('returns a single article for an admin, with tags and readers', function () {
        Sanctum::actingAs(User::find(2));

        $response = $this->getJson('/api/admin/knowledgebase/1');
    ...
    test('returns 404 for an unknown id', function () {
        ...
        $response = $this->getJson('/api/admin/knowledgebase/999');
...
function resetGetAdminKnowledgebaseItemTestData(): void {
