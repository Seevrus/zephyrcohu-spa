...
describe('Get Admin Offer Item', function () {
    beforeEach(function () {
        Carbon::setTestNowAndTimezone('2026-02-28 21:59:40', 'Europe/Budapest');
        resetGetAdminOfferItemTestData();
    });

    test('returns a single offer, even if unpublished', function () {
        Sanctum::actingAs(User::find(2));

        $response = $this->getJson('/api/admin/offers/2');
    ...
    test('returns 404 for an unknown id', function () {
        ...
        $response = $this->getJson('/api/admin/offers/999');
    ...
    test('returns 404 for a guest', function () {
        $response = $this->getJson('/api/admin/offers/1');
...
function resetGetAdminOfferItemTestData(): void {
